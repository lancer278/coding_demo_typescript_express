# coding_demo_typescript_express
coding_demo_typescript_express

Demonstration of node and javascript skills.  

Accesses Google maps places API to search for location.  Returns details with additional UM specific data.

# install

git clone https://github.com/lancer278/coding_demo_typescript_express
npm install


# configure
see .env.example file.  Create a .env with at least a Google API APIKEY
...
The API will run on port 3000 on the machine.

The path you need is /getGoogleCoords

Pass address parameter to the URL.

e.g.

 localhost:3000/getGoogleCoords?address=EC1R5DF
